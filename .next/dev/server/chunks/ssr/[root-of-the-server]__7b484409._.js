...
function NewProductPage() {
    ...
    const handlePublish = async ()=>{
        // Remove this check or base it only on productData
        // We'll check required fields after preparing data
        setIsPublishing(true);
        try {
            const productData = await prepareProductData();
            console.log("🚀 Prepared raw product data:", productData);
            // NOW validate required fields using productData only
            ...
            const hasFiles = (productData.images?.length || 0) + (productData.pdfs?.length || 0) > 0;
            if (hasFiles) {
                showAlert("success", "📤 Uploading files to server...");
                const uploadForm = new FormData();
                (productData.images || []).forEach((file)=>uploadForm.append("images", file));
                (productData.pdfs || []).forEach((file)=>uploadForm.append("documents", file));
                const uploadResponse = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$axiosClient$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"].post("/product-images", uploadForm, {
                    headers: {
                        "Content-Type": "multipart/form-data"
                    }
                });
                ...
                uploadedImages = serverImages.map((img, index)=>({
                        url: img.url,
                        s3_key: img.s3_key || img.key,
                        folder: img.folder || "product-images",
                        type: "gallery",
                        name: img.name || img.filename || img.original_name,
                        size: img.size,
                        mime_type: img.mime_type || img.mimetype,
                        position: index + 1
                    }));
                uploadedDocuments = serverDocuments.map((doc)=>({
                        url: doc.url,
                        s3_key: doc.s3_key || doc.key,
                        folder: doc.folder || "product-documents",
                        name: doc.name || doc.filename || doc.original_name,
                        size: doc.size,
                        mime_type: doc.mime_type || doc.mimetype
                    }));
                showAlert("success", `✅ Uploaded ${uploadedImages.length} images & ${uploadedDocuments.length} documents`);
            }
            // Use only productData for everything
            ...
            const finalPayload = {
                name: 'srue',
                sku: productData.sku || generateSafeSku(productName),
                category_id: categoryId || 1,
                category: categoryName || "mobile",
                price: parseFloat(productData.price),
                description: productData.description || null,
                video_url: productData.video_url || null,
                meta_title: productData.meta_title || productName,
                meta_description: productData.meta_description || productData.description?.slice(0, 160) || null,
                keywords: productData.keywords || null,
                url_slug: productData.url_slug || slugify(productName),
                is_active: true,
                status: "published",
                ...
                custom_field_values: productData.customFields || []
            };
            console.log("📦 Final payload:", finalPayload);
            const response = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$axiosClient$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"].post("/products", finalPayload, {
                headers: {
            ...
            const msg = error?.response?.data?.errors ? Object.values(error.response.data.errors).flat().join(" • ") : error?.response?.data?.message || error.message || "Unknown error";
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7b484409._.js.map